...
class ApiRequester:
    def __init__(self, authkey_path, authkey_type, echo=True):
        self.urllist = []
        self.type = authkey_type
        with open("configs/api_preset.json", "r", encoding="utf-8") as json_file:
            key_list = json.load(json_file)
            self.base_url = key_list["api_key_list"][authkey_type]["base_url"]
            self.authkey_name = key_list["api_key_list"][authkey_type]["authkey_name"]
    ...
    def log(self, content):
        if self.echo:
            print('ApiRequester {}> \033[92m{}'.format(
                self.type, content) + '\033[0m')

    def loadInput(self, input_path):
        with open(input_path, "r", encoding="utf-8") as json_file:
            input = json.load(json_file)
            self.setInputs(input['input_base'],
                           input['list_key'], input['list_val'])
    ...
    def setInput(self, input, is_direct=True):
        ...
        self.urllist.append('{}?{}={}&format=xml&{}'.format(
            self.base_url, self.authkey_name, self.authkey, parse.urlencode(new_dict)))
        if is_direct:
    ...
    def _dictExtractorByKeys(self, data, keys, is_parent=True):
        if is_parent == True:
            ...
            for each_data in data:
                data_list.extend(self._dictExtractorByKeys(
    ...
    def getOutputs(self, keys=-1, output_type='json'):  # or csv
        if keys == -1:
            ...
            with open(self.save_name_raw, "w", encoding="utf-8") as json_file:
